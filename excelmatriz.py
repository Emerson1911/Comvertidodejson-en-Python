...
def normalize_text(text):
    """
    Normaliza el texto para hacer comparaciones más consistentes
    """
    if not text:
        return ""
...
def get_processed_comments(comment_text):
    """
    Procesa y retorna tanto el comentario completo como el segundo comentario
    """
    if not comment_text:
        return "", ""
        
    full_comment = str(comment_text).strip()
    parts = re.split(r'[\n;]+', full_comment)
    second_comment = parts[1].strip() if len(parts) > 1 else parts[0].strip()
    
    return full_comment, second_comment
    ...
    for row in range(1, sheet.max_row + 1):
        for col in range(1, sheet.max_column + 1):
            ...
            if cell.comment and extract_number(cell.value) is not None:
                ...
                # Obtener tanto el comentario completo como el segundo comentario
                full_comment, second_comment = get_processed_comments(comment_text)
                
                # Encontrar ciudades mencionadas en el comentario usando el segundo comentario para la búsqueda
                cities_in_comment = find_route_in_comment(second_comment, cities)
                    ...
                    if destination_cities:  # Si hay ciudades destino
                        for dest_city in destination_cities:
                            routes.append({
                                ...
                                "comentario_completo": full_comment,
                                "comentario_procesado": second_comment,
...
# Lista de ciudades [mantener la misma lista que tenías antes]
ciudades = [
    "AGUADULCE",
    "AHUACHAPAN",
    "ALAJUELA",
